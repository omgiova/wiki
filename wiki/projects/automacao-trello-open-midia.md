...
- **Mensagens começam com 🟡** (círculo amarelo — decisão do Giovani em 2026-07-18, pra diferenciar do 🟢 do Fluxo 1). Tempo real: `🟡 Oi, <apelido>, você foi mencionad<o/a> em um comentário no card *X* por *Fulano*` + Cliente (mesmo mapa de nomes limpos do Fluxo 1) + Prazo + link. Fila: uma mensagem agrupada por pessoa, cada menção com timestamp (`por *Giovani* (17/07 às 23h14)`). **Sem o texto do comentário** em nenhum dos dois formatos (decisão do Giovani em 2026-07-18); o texto ainda é capturado e guardado na coluna `comentario` da fila, só não entra na mensagem.
...
- **Edições pós-criação (2026-07-18):** troca do 🟢 pelo 🟡 nas 5 mensagens (4 nós `Enviar texto` + Code `Montar mensagem por membro`) via PUT, resto devolvido verbatim; depois, remoção da linha 💬 (texto do comentário) das mesmas 5 mensagens, também via PUT com o resto verbatim (200 OK).
